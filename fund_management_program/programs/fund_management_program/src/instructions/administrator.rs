...
pub fn set_nav(ctx: Context<FundAdministratorAccount>, valuation: I80F48) -> Result<()> {
    require!(ctx.accounts.fund.state == FundState::Open, FundManagementError::FundIsClosed);
    require_keys_eq!(ctx.accounts.fund.fund_administrator, ctx.accounts.administrator.key(), FundManagementError::FundIsClosed);
